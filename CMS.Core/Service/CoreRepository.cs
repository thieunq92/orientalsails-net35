...
//				}
		...
		/// for example CMS.Core.Domain.Node.Sections.</param>
		...
		public virtual IList FindUsersByUsername(string searchString)
		{
            ...
            if (searchString.Length > 0)
            {
                ...
                hql = "from User u where u.UserName like :userName order by u.UserName ";
        ...
        public virtual IList GetSystemSettings()
        {
            if (_activeSession == null)
            {
                _activeSession = _factory.OpenSession();
            }
            ICriteria crit = this._activeSession.CreateCriteria(typeof(SystemSetting));
            crit.Add(Expression.IsNull(SystemSetting.MODULETYPE));
            return crit.List();
        }

        public virtual IList GetModuleSettings(ModuleType moduleType)
        {
            if (_activeSession == null)
            {
                _activeSession = _factory.OpenSession();
            ...
            ICriteria crit = this._activeSession.CreateCriteria(typeof(SystemSetting));
            crit.Add(Expression.Eq(SystemSetting.MODULETYPE, moduleType));
            return crit.List();
        }
        #endregion
    }
